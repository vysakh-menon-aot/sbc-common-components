    ...
    def enable_tracing(function):
        ...
        def wrapper(*func_args, **func_kwargs):
            ...
            with (tracer.start_active_span(function.__name__)) as scope:
                span = scope.span
                span.set_tag(TraceTags.COMPONENT, 'Service')
                span.log_kv(
                    {
                        TraceTags.CLASS_NAME: ServiceTracing._get_class_name_that_defined_method(function),
                        TraceTags.FUNCTION_NAME: function.__name__,
                        TraceTags.FUNCTION_ARGS: ', '.join('%s' % p for p in func_args),
                        TraceTags.FUNCTION_KWARGS: ', '.join(['{}={!r}'.format(k, v) for k, v in func_kwargs.items()]),
                    }
                )
                ...
                if retval is not None:
                    with (tracer.start_active_span('{0}.{1}'.format(function.__name__, 'response'))) as ret_scope:
                        ...
                        ret_span.log_kv({TraceTags.FUNCTION_RESPONSE: retval.__dict__})
